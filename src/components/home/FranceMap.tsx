...
export default function FranceMap({
  regions, locale, caption, unitOne, unitMany, none,
}: { regions: RegionNode[]; locale: string; caption: string; unitOne: string; unitMany: string; none: string }) {
  ...
  const tipTop = active ? ((active.cy + PAD - (active.count ? radius(active.count) : 6)) / (h + 2 * PAD)) * 100 : 0;

  return (
    <figure className="m-0 fr-map relative">
      <svg ref={svgRef} viewBox={`${-PAD} ${-PAD} ${w + 2 * PAD} ${h + 2 * PAD}`} className="w-full h-auto overflow-visible" role="img" aria-label={caption}>
        ...
        {shapes.filter((s) => s.count > 0).map((s) => {
          const r = radius(s.count);
          return (
            <Link
              key={s.code}
              href={`/${locale}/carte?region=${s.code}`}
              className={`fr-node ${hover === s.code ? "is-hot" : ""}`}
              style={{ animationDelay: `${900 + s.i * 55}ms` }}
              onMouseEnter={() => setHover(s.code)}
              onMouseLeave={() => setHover(null)}
              onFocus={() => setHover(s.code)}
              onBlur={() => setHover(null)}
              aria-label={`${s.name} : ${s.count} ${s.count > 1 ? unitMany : unitOne}`}
            >
              <circle cx={s.cx} cy={s.cy} r={r + 7} className="fr-halo" />
